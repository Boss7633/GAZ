...
const LivreurApp: React.FC<LivreurAppProps> = ({ profile }) => {
  ...
  const startTracking = () => {
    ...
    update();
    trackInterval.current = setInterval(update, 10000);
  };
    ...
    if (currentPos) {
      L.marker(currentPos, {
        icon: L.divIcon({
          ...
          html: `<div class="bg-blue-600 p-2 rounded-full border-2 border-white text-white shadow-lg animate-pulse"><svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" fill="none" stroke-width="3"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="3"></circle></svg></div>`,
    ...
    orders.forEach(order => {
      ...
      if (cPos) {
        L.marker(cPos, {
          icon: L.divIcon({
            className: 'custom-div-icon',
            html: `<div class="bg-emerald-600 p-2 rounded-full border-2 border-white text-white shadow-lg"><svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" fill="none" stroke-width="3"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg></div>`,
            iconSize: [30, 30], iconAnchor: [15, 30]
          })
        }).bindPopup(`<b>Client:</b> ${order.client?.full_name || 'Inconnu'}`).addTo(markersLayerRef.current);
        points.push(cPos);
      }
    });
      ...
      <div className="w-full max-w-md bg-white min-h-screen flex flex-col shadow-2xl relative overflow-hidden">
        <div className="bg-white/80 backdrop-blur-md px-6 pt-12 pb-4 border-b flex justify-between items-center z-20 sticky top-0">
          <div>
            ...
            <p className="text-[10px] font-black uppercase text-emerald-500">{isOnline ? 'GPS Actif - En ligne' : 'Mode Hors-ligne'}</p>
          </div>
          <button onClick={() => {
            const next = !isOnline;
            setIsOnline(next);
            ...
            <div className="h-full w-full absolute inset-0">
               ...
               <div className="absolute bottom-6 left-6 right-6 z-10 space-y-3">
                 {orders.filter(o => o.livreur_id === profile.id).map(o => (
                   <div key={o.id} className="bg-white p-4 rounded-3xl shadow-2xl border border-gray-100 flex items-center justify-between">
                     <div className="flex items-center gap-3">
                       ...
                       <div>
                         ...
                         <p className="text-[10px] font-black text-gray-400 uppercase">En route vers client</p>
                       </div>
                     </div>
                     <button onClick={() => openInExternalMap(o)} className="bg-gray-900 text-white p-3 rounded-xl shadow-lg"><ExternalLink size={18} /></button>
                   </div>
                 ))}
               </div>
            </div>
            ...
            <div className="p-6 space-y-6 pb-32">
              {orders.length === 0 && (
                <div className="text-center py-20 opacity-30">
                  ...
                  <p className="font-black uppercase text-xs tracking-widest">Aucune mission disponible</p>
                </div>
              )}
              {orders.map(order => (
                <div key={order.id} className={`bg-white rounded-[32px] p-6 shadow-sm border transition-all ${order.livreur_id === profile.id ? 'border-blue-200 bg-blue-50/30' : 'border-gray-100'}`}>
                  ...
                  <div className="flex gap-2">
                    {order.status === 'PENDING' ? (
                      <button onClick={() => updateOrderStatus(order.id, OrderStatus.ASSIGNED)} className="flex-1 bg-gray-900 text-white py-4 rounded-2xl font-black shadow-lg">Accepter</button>
                    ) : order.status === 'ASSIGNED' ? (
                      <>
                        <button onClick={() => updateOrderStatus(order.id, OrderStatus.IN_PROGRESS)} className="flex-1 bg-blue-600 text-white py-4 rounded-2xl font-black">Démarrer</button>
                      ...
                      <button onClick={() => updateOrderStatus(order.id, OrderStatus.ARRIVED)} className="flex-1 bg-emerald-600 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-2">
                        <CheckCircle2 size={20} /> Je suis arrivé
                      </button>
                    )}
                  </div>
                </div>
             ...
             <div className="p-6 text-center py-20">
               ...
               <h3 className="text-2xl font-black text-gray-900">{profile.wallet_balance?.toLocaleString() || 0} F</h3>
               <p className="text-xs font-black text-gray-400 uppercase tracking-widest mt-1">Solde de mon portefeuille</p>
             </div>
          )}
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-md h-24 border-t flex justify-around items-center px-8 pb-4 sticky bottom-0 z-30">
           ...
           <button onClick={() => setActiveTab('carte')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'carte' ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}>
              <MapIcon size={24} />
              <span className="text-[10px] font-black uppercase tracking-tighter">Navigation</span>
           </button>
